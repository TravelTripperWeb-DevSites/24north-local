...
(function($) {
  ...
  $.fn.RTwidgets = function(options) {
    ...
    if (widget_settings.offer_popup_widget.show) {
      var start = new Date(widget_settings.offer_popup_widget.start_date);
      var end = new Date(widget_settings.offer_popup_widget.end_date);
      var date = new Date();
          ...
          if (data.special_rates.length > 0) {
            data.special_rates.forEach(function(item) {
              ...
              // if (item.rate_plan_code == _this.settings.offer_popup_widget.rt_offer_code) {
                ...
                var view_template = '<div class="rt-guest-widget pop_offer" ' + nvhide + '>\n\t\t\t\t\t\t\t\t\t\t\t\t<div class="rt-widget-header">\n\t\t\t\t\t\t\t\t\t\t\t\t\t<h3>' + item.rate_plan_name + '</h3>\n\t\t\t\t\t\t\t\t\t\t\t\t\t<span class="widget_popup_close" id="rt_pop_close">&times;</span>\n\t\t\t\t\t\t\t\t\t\t\t\t</div> \n\t\t\t\t\t\t\t\t\t\t\t\t' + detail_offer + '\n\t\t\t\t\t\t\t\t\t\t\t\t<div class="body_content" style="background-image: url(\'' + item.lead_photo_url.yankee_large + '\');"><div class="rt-overlay_widget"><div class="popup_detail"><a class="versatile_popup" id="versatile_popup_geo_text" href="' + item.promo_url + '" target="_blank">' + item.short_description + '</a></div></div></div><a href="' + item.promo_url + '" class="widget-booknow-link versatile_popup" id="versatile_popup_geo" target="_blank">' + _this.settings.offer_popup_widget.btn_label + '</a></div><div class="rt-guest-widget open-offer-popup ' + nvshow + '"><span>&rarr;</span></div>';
